...
function get_list_product($dbh){
    $sql='select id,name,price,img,category,stock,status,description from t_product where status =1 order by id DESC ';
    $res=$dbh->query($sql);
    $rows=$res->fetchAll();
    return $rows;
}
...
function get_item_category($dbh,$category_id){
    $sql='select id,name,price,img,category,stock,status,description from t_product where status =1 and category=? ';
    $res=$dbh->prepare($sql);
